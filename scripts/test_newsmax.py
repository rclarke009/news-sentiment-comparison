...
def test_newsmax_support():
    ...
    try:
        ...
        newsmax_sources = [
            s for s in sources 
            if "newsmax" in s.get("id", "").lower() or "newsmax" in s.get("name", "").lower()
        ]
        ...
        if newsmax_sources:
            ...
            conservative_keywords = ["fox", "breitbart", "blaze", "national-review", "daily-wire"]
            similar = [
                s for s in sources 
                if any(kw in s.get("id", "").lower() or kw in s.get("name", "").lower() 
    ...
    params = {
        "sources": "invalid-source-xyz-123",
        "apiKey": api_key,
        "pageSize": 1
    }
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if data.get("status") != "ok":
                ...
                print(f"     - Conclusion: 'newsmax' is accepted as valid but has no content")
            else:
