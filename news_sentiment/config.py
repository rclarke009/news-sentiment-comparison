...
class LLMConfig(BaseModel):
    ...
    model: str = Field(default="llama-3.1-70b-versatile", description="Model name for Groq")
    openai_model: str = Field(default="gpt-4o-mini", description="Model name for OpenAI")
...
class SourceConfig(BaseModel):
    ...
    liberal_rss: list[RSSSourceConfig] = Field(
        default_factory=lambda: [],
        description="RSS feed sources for liberal news"
    )
...
class Config(BaseModel):
    ...
    def from_env(cls) -> "Config":
        ...
        placeholder_patterns = ["your_groq_api_key_here", "your_key", "your_openai_api_key_here"]
        if groq_key and groq_key.strip().lower() in [p.lower() for p in placeholder_patterns]:
            groq_key = None
        if openai_key and openai_key.strip().lower() in [p.lower() for p in placeholder_patterns]:
            openai_key = None
        
        if not groq_key and not openai_key:
            raise ValueError("Either GROQ_API_KEY or OPENAI_API_KEY must be set (not placeholders)")
        ...
        mongodb_uri = os.getenv("MONGODB_URI", "mongodb://localhost:27017/news_sentiment")
