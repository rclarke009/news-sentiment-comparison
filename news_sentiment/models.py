...
class Headline(BaseModel):
    ...
    uplift_score: Optional[float] = Field(
        None, description="Sentiment uplift score (-5 to +5)"
    )
    keyword_boost: float = Field(
        0.0, description="Additional boost from puff piece keywords"
    )
    final_score: Optional[float] = Field(
        None, description="Final score after keyword boost"
    )
    local_sentiment_score: Optional[float] = Field(
        None, description="Local model sentiment score (-5 to +5)"
    )
    local_sentiment_label: Optional[str] = Field(
        None, description="Local model label (POSITIVE/NEGATIVE)"
    )
    local_sentiment_confidence: Optional[float] = Field(
        None, description="Local model confidence (0.0 to 1.0)"
    )
    score_difference: Optional[float] = Field(
        None, description="Difference between LLM and local scores (LLM - local)"
    )
...
class SideStatistics(BaseModel):
    ...
    positive_percentage: float = Field(
        ..., description="Percentage of headlines with score > 0"
    )
    total_headlines: int = Field(..., description="Total number of headlines")
    most_uplifting: Optional[MostUpliftingStory] = Field(
        None, description="Most uplifting story"
    )
    score_distribution: dict[str, int] = Field(
        default_factory=dict, description="Count of headlines by score range"
    )
    avg_local_sentiment: Optional[float] = Field(
        None, description="Average local model sentiment score (-5 to +5)"
    )
    local_positive_percentage: Optional[float] = Field(
        None, description="Percentage of headlines with positive local score"
    )
