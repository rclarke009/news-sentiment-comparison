...
class RSSFetcher:
    ...
    def fetch_from_rss(
        ...
        try:
            ...
            if feed.bozo:
                logger.warning(
                    f"RSS feed parsing warning for {source_name}: {feed.bozo_exception}"
                )
            ...
            for entry in entries:
                try:
                    headline = self._parse_rss_entry(
                        entry, source_name, source_id, political_side
                    )
                    if headline:
            ...
            logger.error(
                f"Error fetching RSS feed from {source_name} (URL: {feed_url}): {e}",
                exc_info=True,
            )
            return []
        except Exception as e:
            logger.error(
                f"Unexpected error parsing RSS feed from {source_name} (URL: {feed_url}): {e}",
                exc_info=True,
            )
            return []

        logger.info(
            f"Successfully fetched {len(headlines)} headlines from {source_name} RSS feed"
        )
        return headlines
    ...
    def _parse_rss_entry(
        ...
        try:
            ...
            if not title:
                logger.warning(
                    f"Skipping RSS entry with missing title from {source_name}"
                )
                return None
            ...
            if not url:
                logger.warning(
                    f"Skipping RSS entry with missing URL from {source_name}"
                )
                return None
            ...
            if hasattr(entry, "published_parsed") and entry.published_parsed:
                ...
                try:
                    ...
                    logger.warning(
                        f"Failed to parse RSS updated date from {source_name}: {e}"
                    )
            ...
            entry_title = (
                entry.get("title", "unknown")[:50]
                if hasattr(entry, "get")
                else "unknown"
            )
            logger.error(
                f"Error parsing RSS entry from {source_name} (title: '{entry_title}...'): {e}",
                exc_info=True,
            )
            return None

    def fetch_rss_sources(
        self, rss_sources: List[dict], political_side: str
    ) -> List[Headline]:
        """
