...
class SentimentScorer:
    ...
    def __init__(
        ...
        local_scorer: Optional[LocalSentimentScorer] = None,
    ):
        """
        ...
        try:
            ...
            if self.local_scorer is not None:
                try:
                    ...
                    logger.warning(
                        f"Local model scoring failed for '{headline.title[:50]}...': {e}"
                    )
            ...
            logger.error(
                f"Error scoring headline '{headline.title}': {e}", exc_info=True
            )
...
Respond with ONLY a single number between -5 and +5 (e.g., "3.2" or "-1.5"). Do not include any explanation or other text."""
        ...
        try:
            if self.config.llm.provider == "groq":
                response = self.llm_client.chat.completions.create(
                    ...
                    messages=[
                        {
                            "role": "system",
                            "content": "You are a sentiment analysis expert. Respond with only a number.",
                        },
            ...
            logger.error(
                f"LLM API error ({provider}) for headline '{headline.title[:50]}...' (source: {headline.source}): {e}",
                exc_info=True,
            )
            raise
    ...
    def _parse_score(self, content: str) -> float:
        ...
        logger.warning(
            f"Could not parse score from LLM response: '{content}' (length: {len(content)})"
        )
        return 0.0
